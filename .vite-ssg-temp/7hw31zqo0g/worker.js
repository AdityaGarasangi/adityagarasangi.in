...
export default {
  async fetch(request, env) {
    ...
    // ── Origin check (supports comma-separated list in ALLOWED_ORIGINS)
    const origin = request.headers.get('Origin') || '';
    const allowedRaw = env.ALLOWED_ORIGINS || env.ALLOWED_ORIGIN || '';
    const allowedList = allowedRaw.split(',').map(s => s.trim()).filter(Boolean);
    if (allowedList.length && !allowedList.includes(origin)) {
      return corsResponse(JSON.stringify({ ok: false, error: 'Forbidden' }), 403, env, origin);
    }
...
function corsResponse(body, status, env, origin) {
  const allowedRaw = env.ALLOWED_ORIGINS || env.ALLOWED_ORIGIN || '*';
  const allowedList = allowedRaw.split(',').map(s => s.trim()).filter(Boolean);
  const responseOrigin = origin && allowedList.includes(origin) ? origin : (allowedList[0] || '*');
